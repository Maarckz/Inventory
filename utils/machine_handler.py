BOARD_SERIAL_PLACEHOLDERS = {
    '', 'n/a', 'na', 'none', 'null', 'unknown', 'not specified',
    'not available', 'to be filled by o.e.m.', 'to be filled by o.e.m',
    'default string', 'default', 'o.e.m.', 'oem', 'system serial number',
    'serial number', 'chassis serial number', '0', '000', '0000000000',
}

def _clean_board_serial(value):

    raw = str(value or '').strip()
    if raw.lower() in BOARD_SERIAL_PLACEHOLDERS:
        return 'N/A'
    return raw
...
def process_machine_data(raw_data):
    ...
    if 'agent_info' in raw_data:
        processed = {
            ...
            'agent_status_raw': raw_data['agent_info'].get('status', 'unknown'),
    ...
    if raw_data.get('inventory', {}).get('hardware'):
        ...
        processed['board_serial'] = _clean_board_serial(
            hardware.get('board_serial'))
...
def get_machine_fallback(hostname):
    ...
    if not h and full:
        h = HostInventory.query.filter(
            HostInventory.hostname.ilike(full)).first()
    if not h and full:
        label = full.split('.', 1)[0]
        if label != full:
            h = HostInventory.query.filter(
                HostInventory.hostname.ilike(label)).first()
        else:
            h = HostInventory.query.filter(
                HostInventory.hostname.ilike(label + '.%')).first()
    if h:
